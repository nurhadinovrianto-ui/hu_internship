...
@section('title', 'Laporan Akhir Magang (Kampus)')

@section('content')
<div class="row page-titles mx-0">
    <div class="col-sm-6 p-md-0">
        <div class="welcome-text">
            <h4>Pemeriksaan Laporan Akhir Kampus</h4>
            <p class="mb-0">Daftar laporan magang formal akademik mahasiswa bimbingan untuk diperiksa dan dinilai oleh DPL.</p>
        </div>
    </div>
    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="javascript:void(0)">DPL</a></li>
            <li class="breadcrumb-item active"><a href="javascript:void(0)">Laporan Akhir Kampus</a></li>
        </ol>
    </div>
</div>
                    ...
                    <table class="table table-responsive-md table-hover">
                        <thead>
                            <tr>
                                ...
                                <th><strong>Judul Laporan & Versi</strong></th>
                        ...
                        <tbody>
                            @forelse($reports as $rep)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <small class="badge badge-sm bg-primary text-white">Versi {{ $rep->revisions->max('version') ?? 1 }}</small>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            ...
                                            @if($rep->revisions && $rep->revisions->count() > 0)
                                                <button class="btn btn-outline-info btn-sm px-3 font-weight-bold" type="button" data-bs-toggle="modal" data-bs-target="#historyModalDpl{{ $rep->id }}">
                                ...
                                <!-- Inline Collapse Form for DPL Review feedback -->
                                <tr class="collapse" id="feedbackForm{{ $rep->id }}">
                                    <td colspan="5" class="bg-light p-4">
                                        <form action="{{ route('dpl.reports.approve', $rep->id) }}" method="POST">
                                            @csrf
                                            <h6 class="text-dark font-weight-bold mb-3">Evaluasi Laporan Magang Akademik (Kampus):</h6>
                                            
                                            <div class="row">
                                                <div class="col-md-3 mb-3">
                                                    <label class="form-label font-weight-bold">Status Keputusan</label>
                                                    <select name="status" class="form-control" required>
                                                        <option value="dpl_approved" {{ $rep->status === 'dpl_approved' ? 'selected' : '' }}>Setujui Laporan (DPL Approved)</option>
                                                        <option value="revision" {{ $rep->status === 'revision' ? 'selected' : '' }}>Minta Revisi (Revision Required)</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-9 mb-3">
                                                    <label class="form-label font-weight-bold">Catatan / Koreksi Revisi untuk Mahasiswa</label>
                                                    <textarea name="dpl_feedback" class="form-control" rows="3" placeholder="Tuliskan catatan revisi atau komentar persetujuan..." required>{{ $rep->dpl_feedback }}</textarea>
                                                </div>
                                            </div>
                                ...
                                <div class="modal fade" id="historyModalDpl{{ $rep->id }}" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title font-weight-bold">Riwayat Unggahan & Revisi - {{ $rep->student->user->name }}</h5>
                                                ...
                                                @foreach($rep->revisions as $rev)
                                                    <div class="p-3 mb-3 border rounded" style="background-color: #F8FAFC;">
                                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                                            <span class="badge bg-primary text-white">Versi {{ $rev->version }}</span>
                                                        ...
                                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                                            ...
                                                            <a href="{{ asset('storage/' . $rev->file_path) }}" target="_blank" class="text-primary font-weight-bold">Lihat File PDF V{{ $rev->version }}</a>
                                                        </div>
                                                        @if($rev->feedback)
                                                            <div class="p-2 mt-2 rounded" style="background-color: #FEF3C7; border-left: 3px solid #F59E0B;">
                                                                <small class="d-block font-weight-bold text-dark">Catatan DPL:</small>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center text-muted py-5">Belum ada laporan kampus yang diunggah mahasiswa bimbingan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
