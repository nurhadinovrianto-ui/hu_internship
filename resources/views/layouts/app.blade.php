...
<head>
    ...
    <title>@yield('title', 'Dashboard') - {{ \App\Models\Setting::getValue('app_name', 'Sistem Magang') }}</title>
...
<body>
    ...
    <div id="main-wrapper">
        ...
        <div class="nav-header">
            <a href="{{ route('dashboard.redirect') }}" class="brand-logo" style="display: flex; align-items: center; text-decoration: none; position: relative; z-index: 10;">
                @php
                    $appLogo = \App\Models\Setting::getValue('app_logo');
                    $logoUrl = $appLogo ? asset('storage/' . $appLogo) : asset('edumin/images/logo-white.png');
                @endphp
                <img class="logo-abbr" src="{{ $logoUrl }}" alt="Logo" style="max-width: 45px; max-height: 45px; object-fit: contain; position: relative; z-index: 10; filter: drop-shadow(0 2px 5px rgba(0,0,0,0.35));">
                <span class="brand-title text-white font-weight-bold" style="margin-left: 12px; font-size: 1.18rem; letter-spacing: 0.5px; position: relative; z-index: 10;">{{ \App\Models\Setting::getValue('app_short_name', 'SIMANG') }}</span>
            </a>
            ...
            <div class="dlabnav-scroll">
                <ul class="metismenu" id="menu">
                    ...
                    @elseif(auth()->user()->hasRole('supervisor-industri'))
                        ...
                        <li>
                            <a href="{{ route('industry.assessment-criteria.index') }}" class="ai-icon" aria-expanded="false">
                                <i class="la la-sliders-h"></i>
                                <span class="nav-text">Kriteria Penilaian</span>
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('industry.certificates.index') }}" class="ai-icon" aria-expanded="false">
                                <i class="la la-certificate"></i>
                                <span class="nav-text">Sertifikat Magang</span>
                            </a>
                        </li>
                    ...
                    @elseif(auth()->user()->hasRole('mahasiswa'))
                        ...
                        @php
                            ...
                            $activeIntern = $studentData ? $studentData->internships()->latest('id')->first() : null;
                            $hasDplReport = $activeIntern ? $activeIntern->dplReport()->exists() : false;
                        @endphp
                        @if($hasDplReport)
                        <li>
        ...
        <div class="footer">
            <div class="copyright">
                <p>Hak Cipta © {{ now()->year }} Horizon University Indonesia. Template by <span class="font-weight-bold text-primary">Denzoe</span></p>
            </div>
        </div>
        <!-- Footer end -->
